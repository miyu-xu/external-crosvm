...
if [[ -f "cargo2android.json" ]]; then
  ...
  # NOTE: This is imperfect since it only searches direct subdirectories.
  # For example, ./rutabaga_gfx/src/rutabaga_gralloc is missed.
  if compgen -G "*/Cargo.toml" > /dev/null; then
    C2A_ARGS+=" --no-subdir"
  fi
fi
