...
# Run cargo2android with the appropriate arguments.
...
set -x
cargo2android --cfg cargo2android.json --cargo-bin "$CARGO_BIN"
set +x

rm -f cargo.out cargo.metadata
rm -rf target.tmp || /bin/true

# Revert changes to Cargo.lock caused by cargo2android.
#
# Android diffs in Cargo.toml files can cause diffs in the Cargo.lock when
# cargo2android runs. This didn't happen with cargo2android.py because it
