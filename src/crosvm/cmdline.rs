...
pub struct RunCommand {
    ...
    #[argh(switch)]
    /// this allows VM to be loaded in memory carveout/fixed memory, rather than using
    /// runtime-allocated memory by CrosVM.
    /// This is only applicable to Guest VMs running on Gunyah Hypervisor.
    ...
    #[argh(option, long = "vm-name", arg_name = "NAME")]
...
impl TryFrom<RunCommand> for super::config::Config {
    ...
    fn try_from(cmd: RunCommand) -> Result<Self, Self::Error> {
        ...
        if cfg.use_fixed_memory && cmd.vm_name.is_none() {
            return Err("vm_name has to be set to use fixed memory option"
                .to_string(),
            );
        }
