...
"--mem=2048".to_string(),
"--cpus=2".to_string(),
"--params=console=hvc0 panic=-1 earlycon=uart8250,io,0x3f8 pci=noacpi reboot=k androidboot.console=invalid  printk.devkmsg=on firmware_class.path=/vendor/etc/ init=/init androidboot.hardware=cutf_cvm loop.max_part=7 androidboot.fstab_suffix=f2fs buildvariant=userdebug androidboot.hardware.gralloc=minigbm androidboot.hardware.hwcomposer=drm_minigbm androidboot.hardware.egl=angle androidboot.hardware.vulkan=ranchu androidboot.hardware.gltransport=virtio-gpu-asg androidboot.boot_devices=pci0000:00/0000:00:04.0 androidboot.serialno=CUTTLEFISHCVD011 androidboot.lcd_density=160 androidboot.setupwizard_mode=DISABLED audit=1 androidboot.force_normal_boot=1 androidboot.vsock_tombstone_port=6600 androidboot.cuttlefish_config_server_port=6800 androidboot.vendor.vehiclehal.server.cid=2 androidboot.vendor.vehiclehal.server.port=9210 androidboot.vendor.audiocontrol.server.cid=3 androidboot.vendor.audiocontrol.server.port=9410 androidboot.vsock_keymaster_port=7200 androidboot.vsock_gatekeeper_port=7300 androidboot.modem_simulator_ports=9200 androidboot.wifi_mac_address=40:1:2:3:4:1 androidboot.verifiedbootstate=orange ".to_string(),
...
"--serial=hardware=virtio-console,num=2,type=sink".to_string(),
...
"--bios=/mnt/build1t/zero-copy/aosp-master/out/target/product/vsoc_x86/bootloader".to_string(),
    ];
