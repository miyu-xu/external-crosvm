...
pub enum MemoryRegionPurpose {
    ...
    // DO NOT SUBMIT: Probably don't need this as a separate variant. Code can look a at the
    // MemoryRegionOptions if they need to know it is file backed or not.
    /// General purpose guest memory, backed by an arbitrary file.
    GuestMemoryFileBackedRegion,
...
// DO NOT SUBMIT: need to audit this library. some functions might assume all the regions are
// backed by a single shmem object, e.g. `offset_from_base`.
    ...
    pub fn new_with_options(
        ...
        for range in ranges {
            ...
            if let Some(file_backed) = &range.2.file_backed {
                ...
                // DO NOT SUBMIT: error if not writable? probably doesn't make sense for RAM.
                let prot = if file_backed.writable {
