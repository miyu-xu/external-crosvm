...
#[derive(MsgOnSocket, Debug, PartialEq)]
...
pub type DisplayControlRequestSocket = MsgSocket<DisplayControlCommand, DisplayControlResult>;
pub type DisplayControlResponseSocket = MsgSocket<DisplayControlResult, DisplayControlCommand>;
...
impl VmRequest {
    ...
    pub fn execute(
        ...
        display_control_socket: &DisplayControlRequestSocket,
        ...
        match *self {
            ...
            VmRequest::DisplayCommand(ref cmd) => {
                let res = display_control_socket.send(cmd);
                if let Err(e) = res {
                    error!("fail to send command to display control socket: {}", e);
                    return VmResponse::Err(SysError::new(EIO));
                }
                match display_control_socket.recv() {
