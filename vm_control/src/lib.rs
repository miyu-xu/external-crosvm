...
use resources::{Alloc, GpuMemoryDesc, MmioType, NonMappedDeviceMemory, SystemAllocator};
use sync::Mutex;
use sys_util::{
    error, Error as SysError, EventFd, GuestAddress, MemoryMapping, MmapError, Result,
};
...
pub struct NonSocketExpressibleRequests {
    pub non_mapped_memory_requests: Vec<NonMappedDeviceMemory>,
}
...
pub enum VmMemoryRequest {
    ...
    /// address space and requiores that NonSocketExpressibleRequests has exactly 1 pending request
    RegisterPendingHostPointerAtPciBarOffset(Alloc, u64),
    /// Unregister the given memory slot that was previously registered with `RegisterMemory`.
    UnregisterMemory(u32),
    /// Unregister the NonMappedDeviceMemory that was previously registered with
...
impl VmMemoryRequest {
    ...
    pub fn execute(
        ...
        non_socket_expr_reqs: Arc<Mutex<NonSocketExpressibleRequests>>,
    ) -> VmMemoryResponse {
        ...
        match *self {
            ...
            RegisterPendingHostPointerAtPciBarOffset(alloc, offset) => {
                ...
                if 1 != locked_reqs.non_mapped_memory_requests.len() {
                    panic!("Only one NonSocketExpressibleRequest outstanding allowed!");
                }

                let mem = locked_reqs.non_mapped_memory_requests.remove(0);
                let res = match register_memory_hva(vm, sys_allocator, mem, (alloc, offset)) {
                    Ok((pfn, slot)) => VmMemoryResponse::RegisterMemory { pfn, slot },
                    Err(e) => VmMemoryResponse::Err(e),
                };
            ...
            UnregisterHostPointerMemory(slot) => match vm.remove_non_mapped_device_memory(slot) {
                Ok(_) => VmMemoryResponse::Ok,
                Err(e) => VmMemoryResponse::Err(e),
            },
            AllocateAndRegisterGpuMemory {
...
fn register_memory_hva(
    vm: &mut Vm,
    allocator: &mut SystemAllocator,
    mem: NonMappedDeviceMemory,
    allocation: (Alloc, u64),
) -> Result<(u64, u32)> {
    let addr = allocator
        .mmio_allocator(MmioType::High)
        .address_from_pci_offset(allocation.0, allocation.1, mem.size() as u64)
        .map_err(|_e| SysError::new(EINVAL))?;

    let slot = vm.add_non_mapped_device_memory(GuestAddress(addr), mem, false)?;
    Ok((addr >> 12, slot))
}
