...
/// Not everything can be completely communicated over MsgOnSocket; ExternallyMappedHostMemoryRequests
/// holds Rust objects with ownership/lifetime data.  It is intended for there to be only one
/// producer and consumer of ExternallyMappedHostMemoryRequests at a time, in parallel to how
/// VmMemoryControlRequestSocket works one request in flight at a time.  Once a pending device
/// memory mapping request is pushed into ExternallyMappedHostMemoryRequests, the user follows up with
...
pub enum VmMemoryRequest {
    ...
    /// address space and requiores that ExternallyMappedHostMemoryRequests has exactly 1 pending request
