...
bindgen_generate \
    ...
    "${BINDGEN_LINUX_X86_HEADERS}/include/linux/gunyah.h" \
    -- \
    -isystem "${BINDGEN_LINUX_X86_HEADERS}/include" \
    | replace_linux_int_types \
    > gunyah_sys/src/x86/bindings.rs
...
bindgen_generate \
    ...
    --blocklist-item='__FD_SETSIZE' \
    --blocklist-item='_?IOC.*' \
    "${BINDGEN_LINUX_ARM64_HEADERS}/include/linux/gunyah.h" \
    -- \
    -isystem "${BINDGEN_LINUX_ARM64_HEADERS}/include" \
    | replace_linux_int_types \
    > gunyah_sys/src/aarch64/bindings.rs
