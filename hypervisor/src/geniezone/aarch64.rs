...
impl Gz {
    ...
    pub fn get_vm_type(&self, protection_type: ProtectionType) -> Result<u32> {
        // Safe because we know self is a real gz fd
...
impl VmAArch64 for GzVm {
    ...
    fn create_vcpu(&self, id: usize) -> Result<Box<dyn VcpuAArch64>> {
        ...
        Ok(Box::new(GzVm::create_vcpu(self, id)?))
    }
}
...
impl GzVcpu {
    ...
    fn set_one_gz_reg_u64(&self, kvm_reg_id: GzVcpuRegister, data: u64) -> Result<()> {
        self.set_one_gz_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn set_one_gz_reg(&self, kvm_reg_id: GzVcpuRegister, data: &[u8]) -> Result<()> {
    ...
    fn get_one_gz_reg_u64(&self, kvm_reg_id: GzVcpuRegister) -> Result<u64> {
        let mut bytes = 0u64.to_ne_bytes();
        self.get_one_gz_reg(kvm_reg_id, bytes.as_mut_slice())?;
        Ok(u64::from_ne_bytes(bytes))
    }

    fn get_one_gz_reg(&self, kvm_reg_id: GzVcpuRegister, data: &mut [u8]) -> Result<()> {
        let onereg = kvm_one_reg {
            id: kvm_reg_id.into(),
...
#[cfg(feature = "gdb")]
impl GzVcpu {
    fn set_one_kvm_reg_u32(&self, kvm_reg_id: GzVcpuRegister, data: u32) -> Result<()> {
        self.set_one_gz_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn set_one_kvm_reg_u128(&self, kvm_reg_id: GzVcpuRegister, data: u128) -> Result<()> {
        self.set_one_gz_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn get_one_kvm_reg_u32(&self, kvm_reg_id: GzVcpuRegister) -> Result<u32> {
        let mut bytes = 0u32.to_ne_bytes();
        self.get_one_gz_reg(kvm_reg_id, bytes.as_mut_slice())?;
        Ok(u32::from_ne_bytes(bytes))
    }

    fn get_one_kvm_reg_u128(&self, kvm_reg_id: GzVcpuRegister) -> Result<u128> {
    ...
    fn demux_register(&self, reg: &<GdbArch as Arch>::RegId) -> Result<Option<GzVcpuRegister>> {
        match *reg {
            AArch64RegId::CCSIDR_EL1 => {
                let csselr = GzVcpuRegister::try_from(AArch64RegId::CSSELR_EL1)
                    .expect("can't map AArch64RegId::CSSELR_EL1 to GzVcpuRegister");
                if let Ok(csselr) = self.get_one_gz_reg_u64(csselr) {
                    Ok(Some(GzVcpuRegister::Ccsidr(csselr as u8)))
                } else {
                    Ok(None)
                }
            }
...
impl From<GzVcpuRegister> for u64 {
    fn from(register: GzVcpuRegister) -> Self {
        ...
        match register {
            GzVcpuRegister::X(n @ 0..=30) => {
                let n = std::mem::size_of::<u64>() * (n as usize);

                user_pt_reg(memoffset::offset_of!(user_pt_regs, regs) + n)
            }
            GzVcpuRegister::X(n) => unreachable!("invalid GzVcpuRegister Xn index: {n}"),
            GzVcpuRegister::Sp => user_pt_reg(memoffset::offset_of!(user_pt_regs, sp)),
            GzVcpuRegister::Pc => user_pt_reg(memoffset::offset_of!(user_pt_regs, pc)),
            GzVcpuRegister::Pstate => user_pt_reg(memoffset::offset_of!(user_pt_regs, pstate)),
            GzVcpuRegister::SpEl1 => kvm_reg(memoffset::offset_of!(kvm_regs, sp_el1)),
            GzVcpuRegister::ElrEl1 => kvm_reg(memoffset::offset_of!(kvm_regs, elr_el1)),
            GzVcpuRegister::Spsr(n @ 0..=4) => {
                let n = std::mem::size_of::<u64>() * (n as usize);

                kvm_reg(memoffset::offset_of!(kvm_regs, spsr) + n)
            }
            GzVcpuRegister::Spsr(n) => unreachable!("invalid GzVcpuRegister Spsr index: {n}"),
            GzVcpuRegister::V(n @ 0..=31) => {
            ...
            GzVcpuRegister::V(n) => unreachable!("invalid GzVcpuRegister Vn index: {n}"),
            GzVcpuRegister::Fpsr => user_fpsimd_state_reg(
                KVM_REG_SIZE_U32,
                memoffset::offset_of!(user_fpsimd_state, fpsr),
            ),
            GzVcpuRegister::Fpcr => user_fpsimd_state_reg(
                KVM_REG_SIZE_U32,
                memoffset::offset_of!(user_fpsimd_state, fpcr),
            ),
            GzVcpuRegister::Firmware(n) => reg_u64(KVM_REG_ARM_FW.into(), n.into()),
            GzVcpuRegister::System(n) => reg_u64(KVM_REG_ARM64_SYSREG.into(), n.into()),
            GzVcpuRegister::Ccsidr(n) => demux_reg(KVM_REG_SIZE_U32, 0, n.into()),
        }
    }
}
...
#[cfg(feature = "gdb")]
impl TryFrom<AArch64RegId> for GzVcpuRegister {
    type Error = Error;
...
impl From<VcpuRegAArch64> for GzVcpuRegister {
...
impl VcpuAArch64 for GzVcpu {
    ...
    fn set_one_reg(&self, reg_id: VcpuRegAArch64, data: u64) -> Result<()> {
        self.set_one_gz_reg_u64(GzVcpuRegister::from(reg_id), data)
    }

    fn get_one_reg(&self, reg_id: VcpuRegAArch64) -> Result<u64> {
        self.get_one_gz_reg_u64(GzVcpuRegister::from(reg_id))
    }
    ...
    fn set_gdb_registers(&self, regs: &<GdbArch as Arch>::Registers) -> Result<()> {
        ...
        for (i, reg) in regs.x.iter().enumerate() {
            ...
            self.set_one_gz_reg_u64(GzVcpuRegister::X(n), *reg)?;
        }
        self.set_one_gz_reg_u64(GzVcpuRegister::Sp, regs.sp)?;
        self.set_one_gz_reg_u64(GzVcpuRegister::Pc, regs.pc)?;
        // GDB gives a 32-bit value for "CPSR" but KVM wants a 64-bit Pstate.
        let pstate = self.get_one_gz_reg_u64(GzVcpuRegister::Pstate)?;
        let pstate = (pstate & 0xffff_ffff_0000_0000) | (regs.cpsr as u64);
        self.set_one_gz_reg_u64(GzVcpuRegister::Pstate, pstate)?;
        for (i, reg) in regs.v.iter().enumerate() {
            let n = u8::try_from(i).expect("invalid Vn general purpose register index");
            self.set_one_kvm_reg_u128(GzVcpuRegister::V(n), *reg)?;
        }
        self.set_one_kvm_reg_u32(GzVcpuRegister::Fpcr, regs.fpcr)?;
        self.set_one_kvm_reg_u32(GzVcpuRegister::Fpsr, regs.fpsr)?;
    ...
    fn get_gdb_registers(&self, regs: &mut <GdbArch as Arch>::Registers) -> Result<()> {
        ...
        for (i, reg) in regs.x.iter_mut().enumerate() {
            let n = u8::try_from(i).expect("invalid Xn general purpose register index");
            *reg = self.get_one_gz_reg_u64(GzVcpuRegister::X(n))?;
        }
        regs.sp = self.get_one_gz_reg_u64(GzVcpuRegister::Sp)?;
        regs.pc = self.get_one_gz_reg_u64(GzVcpuRegister::Pc)?;
        // KVM gives a 64-bit value for Pstate but GDB wants a 32-bit "CPSR".
        regs.cpsr = self.get_one_gz_reg_u64(GzVcpuRegister::Pstate)? as u32;
        for (i, reg) in regs.v.iter_mut().enumerate() {
            let n = u8::try_from(i).expect("invalid Vn general purpose register index");
            *reg = self.get_one_kvm_reg_u128(GzVcpuRegister::V(n))?;
        }
        regs.fpcr = self.get_one_kvm_reg_u32(GzVcpuRegister::Fpcr)?;
        regs.fpsr = self.get_one_kvm_reg_u32(GzVcpuRegister::Fpsr)?;
    ...
    fn set_gdb_register(&self, reg: <GdbArch as Arch>::RegId, data: &[u8]) -> Result<()> {
        ...
        let kvm_reg = if kvm_multiplexes(&reg) {
            ...
            GzVcpuRegister::try_from(reg)?
        };
        self.set_one_gz_reg(kvm_reg, &data[..len])
    }
    ...
    fn get_gdb_register(&self, reg: <GdbArch as Arch>::RegId, data: &mut [u8]) -> Result<usize> {
        ...
        self.get_one_gz_reg(kvm_reg, &mut data[..len])
            .and(Ok(len))
...
mod tests {
    ...
    fn set_gsi_routing() {
        ...
        let vm = GzVm::new(&kvm, gm, Default::default()).unwrap();
