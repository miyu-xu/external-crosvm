...
impl GeniezoneVm {
    ...
    /// let the arch-independent `GeniezoneVm::create_device` handle it.
...
#[cfg(feature = "gdb")]
impl GeniezoneVcpu {
    fn set_one_geniezone_reg_u32(&self, kvm_reg_id: GeniezoneVcpuRegister, data: u32) -> Result<()> {
        self.set_one_geniezone_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn set_one_geniezone_reg_u128(&self, kvm_reg_id: GeniezoneVcpuRegister, data: u128) -> Result<()> {
        self.set_one_geniezone_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn get_one_geniezone_reg_u32(&self, kvm_reg_id: GeniezoneVcpuRegister) -> Result<u32> {
        let mut bytes = 0u32.to_ne_bytes();
        self.get_one_geniezone_reg(kvm_reg_id, bytes.as_mut_slice())?;
        Ok(u32::from_ne_bytes(bytes))
    }

    fn get_one_geniezone_reg_u128(&self, kvm_reg_id: GeniezoneVcpuRegister) -> Result<u128> {
