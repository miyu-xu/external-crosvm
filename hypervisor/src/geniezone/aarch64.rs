...
impl GeniezoneVcpu {
    ...
    fn set_one_geniezone_reg_u64(&self, kvm_reg_id: GeniezoneVcpuRegister, data: u64) -> Result<()> {
        self.set_one_geniezone_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn set_one_geniezone_reg(&self, kvm_reg_id: GeniezoneVcpuRegister, data: &[u8]) -> Result<()> {
    ...
    fn get_one_geniezone_reg_u64(&self, kvm_reg_id: GeniezoneVcpuRegister) -> Result<u64> {
        let mut bytes = 0u64.to_ne_bytes();
        self.get_one_geniezone_reg(kvm_reg_id, bytes.as_mut_slice())?;
        Ok(u64::from_ne_bytes(bytes))
    }

    fn get_one_geniezone_reg(&self, kvm_reg_id: GeniezoneVcpuRegister, data: &mut [u8]) -> Result<()> {
        let onereg = kvm_one_reg {
            id: kvm_reg_id.into(),
...
#[cfg(feature = "gdb")]
impl GeniezoneVcpu {
    fn set_one_kvm_reg_u32(&self, kvm_reg_id: GeniezoneVcpuRegister, data: u32) -> Result<()> {
        self.set_one_geniezone_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn set_one_kvm_reg_u128(&self, kvm_reg_id: GeniezoneVcpuRegister, data: u128) -> Result<()> {
        self.set_one_geniezone_reg(kvm_reg_id, data.to_ne_bytes().as_slice())
    }

    fn get_one_kvm_reg_u32(&self, kvm_reg_id: GeniezoneVcpuRegister) -> Result<u32> {
        let mut bytes = 0u32.to_ne_bytes();
        self.get_one_geniezone_reg(kvm_reg_id, bytes.as_mut_slice())?;
        Ok(u32::from_ne_bytes(bytes))
    }

    fn get_one_kvm_reg_u128(&self, kvm_reg_id: GeniezoneVcpuRegister) -> Result<u128> {
...
impl From<GeniezoneVcpuRegister> for u64 {
    fn from(register: GeniezoneVcpuRegister) -> Self {
        ...
        match register {
            ...
            GeniezoneVcpuRegister::SpEl1 => kvm_reg(memoffset::offset_of!(kvm_regs, sp_el1)),
            GeniezoneVcpuRegister::ElrEl1 => kvm_reg(memoffset::offset_of!(kvm_regs, elr_el1)),
            ...
            GeniezoneVcpuRegister::Firmware(n) => reg_u64(KVM_REG_ARM_FW.into(), n.into()),
            GeniezoneVcpuRegister::System(n) => reg_u64(KVM_REG_ARM64_SYSREG.into(), n.into()),
            GeniezoneVcpuRegister::Ccsidr(n) => demux_reg(KVM_REG_SIZE_U32, 0, n.into()),
        }
    }
}
...
impl VcpuAArch64 for GeniezoneVcpu {
    ...
    fn set_gdb_registers(&self, regs: &<GdbArch as Arch>::Registers) -> Result<()> {
        ...
        for (i, reg) in regs.x.iter().enumerate() {
            ...
            self.set_one_geniezone_reg_u64(GeniezoneVcpuRegister::X(n), *reg)?;
        }
        self.set_one_geniezone_reg_u64(GeniezoneVcpuRegister::Sp, regs.sp)?;
        self.set_one_geniezone_reg_u64(GeniezoneVcpuRegister::Pc, regs.pc)?;
        // GDB gives a 32-bit value for "CPSR" but KVM wants a 64-bit Pstate.
        let pstate = self.get_one_geniezone_reg_u64(GeniezoneVcpuRegister::Pstate)?;
        let pstate = (pstate & 0xffff_ffff_0000_0000) | (regs.cpsr as u64);
        self.set_one_geniezone_reg_u64(GeniezoneVcpuRegister::Pstate, pstate)?;
        for (i, reg) in regs.v.iter().enumerate() {
            let n = u8::try_from(i).expect("invalid Vn general purpose register index");
            self.set_one_kvm_reg_u128(GeniezoneVcpuRegister::V(n), *reg)?;
        }
        self.set_one_kvm_reg_u32(GeniezoneVcpuRegister::Fpcr, regs.fpcr)?;
        self.set_one_kvm_reg_u32(GeniezoneVcpuRegister::Fpsr, regs.fpsr)?;
    ...
    fn get_gdb_registers(&self, regs: &mut <GdbArch as Arch>::Registers) -> Result<()> {
        ...
        for (i, reg) in regs.x.iter_mut().enumerate() {
            let n = u8::try_from(i).expect("invalid Xn general purpose register index");
            *reg = self.get_one_geniezone_reg_u64(GeniezoneVcpuRegister::X(n))?;
        }
        regs.sp = self.get_one_geniezone_reg_u64(GeniezoneVcpuRegister::Sp)?;
        regs.pc = self.get_one_geniezone_reg_u64(GeniezoneVcpuRegister::Pc)?;
        // KVM gives a 64-bit value for Pstate but GDB wants a 32-bit "CPSR".
        regs.cpsr = self.get_one_geniezone_reg_u64(GeniezoneVcpuRegister::Pstate)? as u32;
        for (i, reg) in regs.v.iter_mut().enumerate() {
            let n = u8::try_from(i).expect("invalid Vn general purpose register index");
            *reg = self.get_one_kvm_reg_u128(GeniezoneVcpuRegister::V(n))?;
        }
        regs.fpcr = self.get_one_kvm_reg_u32(GeniezoneVcpuRegister::Fpcr)?;
        regs.fpsr = self.get_one_kvm_reg_u32(GeniezoneVcpuRegister::Fpsr)?;
    ...
    fn set_gdb_register(&self, reg: <GdbArch as Arch>::RegId, data: &[u8]) -> Result<()> {
        ...
        let kvm_reg = if kvm_multiplexes(&reg) {
            ...
            GeniezoneVcpuRegister::try_from(reg)?
        };
        self.set_one_geniezone_reg(kvm_reg, &data[..len])
    }
    ...
    fn get_gdb_register(&self, reg: <GdbArch as Arch>::RegId, data: &mut [u8]) -> Result<usize> {
        ...
        self.get_one_geniezone_reg(kvm_reg, &mut data[..len])
            .and(Ok(len))
...
mod tests {
    ...
    fn set_gsi_routing() {
        ...
        let vm = GeniezoneVm::new(&kvm, gm, Default::default()).unwrap();
