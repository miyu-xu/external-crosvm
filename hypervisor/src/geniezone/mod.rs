...
unsafe fn set_user_memory_region(
    ...
    let mut flags = if read_only { GZVM_MEM_READONLY } else { 0 };
    if log_dirty_pages {
        flags |= GZVM_MEM_LOG_DIRTY_PAGES;
    }
...
impl GeniezoneVm {
    ...
    fn create_vcpu(&self, id: usize) -> Result<GeniezoneVcpu> {
        ...
        let fd = unsafe { ioctl_with_val(self, GZVM_CREATE_VCPU(), c_ulong::try_from(id).unwrap()) };
        if fd < 0 {
            return errno_result();
        }
    ...
    fn check_raw_capability(&self, capability: GeniezoneCap) -> bool {
        ...
        unsafe { ioctl_with_val(self, GZVM_CHECK_EXTENSION(), capability as c_ulong) == 1 }
    }
    ...
    pub fn create_geniezone_device(&self, dev: gzvm_create_device) {
...
impl Vm for GeniezoneVm {
    ...
    fn get_dirty_log(&self, slot: MemSlot, dirty_log: &mut [u8]) -> Result<()> {
        ...
        let mut dirty_log_gzvm = gzvm_dirty_log {
            slot,
            ..Default::default()
        };
        dirty_log_gzvm.__bindgen_anon_1.dirty_bitmap = dirty_log.as_ptr() as *mut c_void;
        ...
        let ret = unsafe { ioctl_with_ref(self, GZVM_GET_DIRTY_LOG(), &dirty_log_gzvm) };
        if ret == 0 {
            Ok(())
        } else {
            errno_result()
...
impl Vcpu for GeniezoneVcpu {
    ...
    fn run(&mut self, run_handle: &VcpuRunHandle) -> Result<VcpuExit> {
        ...
        match run.exit_reason {
            ...
            GZVM_EXIT_EXCEPTION => Ok(VcpuExit::Exception),
