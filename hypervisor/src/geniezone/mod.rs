...
impl Gz {
    pub fn new_with_path(device_path: &Path) -> Result<Gz> {
        ...
        Ok(Gz {
            gz: unsafe { SafeDescriptor::from_raw_descriptor(ret) },
        })
    }

    /// Opens `/dev/kvm/` and returns a Kvm object on success.
    pub fn new() -> Result<Gz> {
        Gz::new_with_path(&PathBuf::from("/dev/gzvm"))
    }
...
impl AsRawDescriptor for Gz {
    fn as_raw_descriptor(&self) -> RawDescriptor {
        self.gz.as_raw_descriptor()
    }
}

impl Hypervisor for Gz {
    fn try_clone(&self) -> Result<Self> {
        Ok(Gz {
            gz: self.gz.try_clone()?,
        })
    }
...
impl GzVm {
    ...
    pub fn new(gz: &Gz, guest_mem: GuestMemory, cfg: Config) -> Result<GzVm> {
    ...
    fn create_vcpu(&self, id: usize) -> Result<GzVcpu> {
        // run is a data stucture shared with ko and gz
        let run_mmap_size = self.gz.get_vcpu_mmap_size()?;
        ...
        let run = unsafe { &mut *(run_mmap.as_ptr() as *mut gzvm_vcpu_run) };
    ...
    pub fn create_gz_device(&self, dev: gzvm_create_device) {
        let ret = unsafe { base::ioctl_with_ref(self, KVM_CREATE_DEVICE(), &dev) };
    }
}
...
impl Vm for GzVm {
    ...
    fn get_guest_phys_addr_bits(&self) -> u8 {
        self.gz.get_guest_phys_addr_bits()
    }
...
impl Vcpu for GzVcpu {
    ...
    fn set_local_immediate_exit_fn(&self) -> extern "C" fn() {
        extern "C" fn f() {
            GzVcpu::set_local_immediate_exit(true);
        }
        f
    }
    ...
    fn run(&mut self, run_handle: &VcpuRunHandle) -> Result<VcpuExit> {
        ...
        let ret = unsafe { ioctl_with_val(self, KVM_RUN(), self.run_mmap.as_ptr() as u64)};
        if ret != 0 {
            return errno_result();
        }
    ...
    fn handle_mmio(&self, handle_fn: &mut dyn FnMut(IoParams) -> Option<[u8; 8]>) -> Result<()> {
        ...
        let mmio = unsafe { &mut run.anon_union.mmio };
...
impl AsRawDescriptor for GzVcpu {
    fn as_raw_descriptor(&self) -> RawDescriptor {
