...
fn read_reg_u64(data: Vec<u8>) -> Result<u64> {
    data.as_slice()
...
impl VcpuAArch64 for KvmVcpu {
    ...
    fn set_gdb_registers(&self, regs: &<GdbArch as Arch>::Registers) -> Result<()> {
        ...
        let pstate = (read_reg_u64(pstate.data)? & 0xffff_ffff_0000_0000) | (regs.cpsr as u64);
        self.set_one_kvm_reg(AArch64Register {
            reg_id: Pstate,
    ...
    fn get_gdb_registers(&self, regs: &mut <GdbArch as Arch>::Registers) -> Result<()> {
        ...
        for (i, reg) in regs.x.iter_mut().enumerate() {
            let n = u8::try_from(i).expect("invalid Xn general purpose register index");
            *reg = read_reg_u64(self.get_one_kvm_reg(X(n))?.data)?;
        }
        regs.sp = read_reg_u64(self.get_one_kvm_reg(SpEl(0))?.data)?;
        regs.pc = read_reg_u64(self.get_one_kvm_reg(Pc)?.data)?;
        // KVM gives a 64-bit value for Pstate but GDB wants a 32-bit "CPSR".
        regs.cpsr = read_reg_u64(self.get_one_kvm_reg(Pstate)?.data)? as u32;
