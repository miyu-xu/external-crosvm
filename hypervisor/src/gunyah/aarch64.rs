...
impl VmAArch64 for GunyahVm {
    ...
    fn create_fdt(
        ...
        let mut fw_address: Option<GuestAddress> = None;
        self.guest_mem.with_regions(|index, guest_addr, size, host_addr, _, _| {
            if self.hv_cfg.protection_type.runs_firmware() {
                if index == 0 {
                    fw_address = Some(guest_addr);
                } else if index == 1 {
        ...
        if let Some(guest_addr) = fw_address {
        ...
        self.guest_mem.with_regions(|label, guest_addr, _, _, _, _| {
            if self.hv_cfg.protection_type.runs_firmware() {
                if label == 0 || label == 1 {
                    return Ok(());
                }
        ...
        self.set_dtb_config(fdt_address, fdt_size).unwrap();
        Ok(())
    }
}
