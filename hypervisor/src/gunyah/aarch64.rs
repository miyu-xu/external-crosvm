...
fn fdt_create_shm_device(
    ...
    fdt.property_u32("label", index.try_into().unwrap())?;
...
impl VmAArch64 for GunyahVm {
    ...
    fn init_arch(
        ...
        // memory parcel. This parcel contains both DTB and kernel Image, so make
        // sure that DTB and payload are in the same memory region and that payload is
        // at the start of that region.
