...
impl VmAArch64 for GunyahVm {
    ...
    fn create_fdt(&self, fdt: &mut FdtWriter) -> cros_fdt::Result<()> {
        ...
        self.guest_mem
            .with_regions(|index, guest_addr, _, _, _, _, options| {
        ...
        self.guest_mem
            .with_regions(|label, guest_addr, _, _, _, _, _| {
                if self.hv_cfg.protection_type == ProtectionType::Protected {
                    // Running firmware that hypervisor loads?
                    // Skip adding main memory and the firmware memory parcels, which
                    // Gunyah is informed about via base-address and firmware-address properties
                    if label == 0 || label == 1 {
                        return Ok(());
                    }
                } else {
                    // Skip main memory parcel; Gunyah knows about it by loading
                    // VM itself.
                    if label == 0 {
                        return Ok(());
                    }
    ...
    fn init_vm(
        ...
        let payload_address = payload.entry().offset();
        ...
        let (payload_mapping, payload_offset, payload_obj_offset) = self
            .guest_mem
            .find_region(GuestAddress(payload.entry().offset()))
            .map_err(|_| Error::new(ENOENT))?;

        if !std::ptr::eq(dtb_mapping, payload_mapping) || dtb_obj_offset != payload_obj_offset {
            panic!("DTB and payload are not part of same memory parcel.");
        }
