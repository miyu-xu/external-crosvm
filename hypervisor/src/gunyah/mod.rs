...
use std::{
    ...
    ffi::CString,
    mem::ManuallyDrop,
    os::{raw::{c_ulong, c_void}, unix::prelude::OsStrExt},
    path::{Path, PathBuf},
...
use base::{
    errno_result, error, ioctl, ioctl_with_ref, ioctl_with_val, pagesize, sys::BlockedSignal,
    Error, FromRawDescriptor, MemoryMapping, MemoryMappingBuilder, MemoryMappingBuilderUnix,
    MmapError, RawDescriptor, warn, info,
};
use libc::{open, EBUSY, EFAULT, EINVAL, EIO, ENOENT, ENOSPC, EOVERFLOW, O_CLOEXEC, O_RDWR, ENOTSUP};
use sync::Mutex;
...
impl GunyahVm {
    pub fn new(gh: &Gunyah, guest_mem: GuestMemory, cfg: Config) -> Result<GunyahVm> {
        ...
        guest_mem.with_regions(|index, guest_addr, size, host_addr, _, _, host_access| {
            unsafe {
    ...
    fn create_vcpu(&self, id: usize) -> Result<GunyahVcpu> {
        let mut function_desc = gh_vm_function {
            name: Default::default(),
            __bindgen_anon_1: gh_vm_function__bindgen_ty_1 {
                vcpu: gh_fn_vcpu_arg {
                    vcpu_id: id as u32,
                },
            },
        ...
        for (f, src) in function_desc.name.iter_mut().zip(b"vcpu\0") {
            *f = *src as _;
        }
    ...
    pub fn register_irqfd(&self, label: u32, evt: &Event, level: bool) -> Result<()> {
        ...
        for (f, src) in function_desc.name.iter_mut().zip(b"irqfd\0") {
            *f = *src as _;
        }

        if level {
            // Safe because we know function_desc is being used to describe an irqfd (above)
            unsafe { function_desc.__bindgen_anon_1.irqfd.flags |= GH_IRQFD_LEVEL; }
        }

        let ret = unsafe { ioctl_with_ref(self, GH_VM_ADD_FUNCTION(), &function_desc) };
        if ret == 0 {
            self.routes.lock().insert(GunyahIrqRoute {irq: label, level});
            Ok(())
        } else {
            errno_result()
        }
    ...
    pub fn unregister_irqfd(&self, label: u32, evt: &Event) -> Result<()> {
        todo!()
    }
    ...
    fn set_dtb_config(&self, fdt_address: GuestAddress, fdt_size: usize) -> Result<()> {
        let dtb_config = gh_vm_dtb_config {
            gpa: fdt_address.offset(),
            size: fdt_size.try_into().unwrap(), // crosvm TODO: not unwrap
        };
...
impl Vm for GunyahVm {
    ...
    fn check_capability(&self, c: VmCap) -> bool {
        match c {
            ...
            VmCap::PvClock => false, // Strictly speaking, yes but not exposed to userspace for crosvm
    ...
    fn register_ioevent(
        ...
        let maddr = if let IoEventAddress::Mmio(maddr) = addr {maddr} else {todo!()};

        let mut function_desc = gh_vm_function {
            name: Default::default(),
            __bindgen_anon_1: gh_vm_function__bindgen_ty_1 {
                ioeventfd: gh_fn_ioeventfd_arg {
        ...
        for (f, src) in function_desc.name.iter_mut().zip(b"ioeventfd\0") {
            *f = *src as _;
        }
...
impl Vcpu for GunyahVcpu {
    ...
    fn run(&mut self, run_handle: &VcpuRunHandle) -> Result<VcpuExit> {
        ...
        match run.exit_reason {
            GH_VM_EXIT_MMIO => Ok(VcpuExit::Mmio),
            r => {
    ...
    fn handle_mmio(&self, handle_fn: &mut dyn FnMut(IoParams) -> Option<[u8; 8]>) -> Result<()> {
        ...
        assert!(run.exit_reason == GH_VM_EXIT_MMIO);
