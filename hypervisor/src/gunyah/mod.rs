...
impl GunyahVm {
    pub fn new(gh: &Gunyah, guest_mem: GuestMemory, cfg: Config) -> Result<GunyahVm> {
        ...
        guest_mem.with_regions(|index, guest_addr, size, host_addr, backing, _| {
            match backing {
                BackingObject::Shm(_) | BackingObject::File(_) => {
                    unsafe {
    ...
    fn create_vcpu(&self, id: usize) -> Result<GunyahVcpu> {
        ...
        let mut function_desc = gh_fn_desc {
            type_: GH_FN_VCPU,
    ...
    fn run(&mut self, run_handle: &VcpuRunHandle) -> Result<VcpuExit> {
        ...
        match run.exit_reason {
            ...
            GH_VCPU_EXIT_STATUS => {
                ...
                match status.status {
                    gh_vm_status_GH_VM_STATUS_LOAD_FAILED => Ok(VcpuExit::FailEntry { hardware_entry_failure_reason: 0}),
