...
/// DeviceMemoryMapping represents an existing buffer in the current address space for purposes
...
/// DeviceMemoryMapping uses a trait object `DeviceMemoryMapping` that performs device
...
pub trait DeviceMemoryMappingInfo {
    fn as_ptr(&self) -> *mut u8;
    fn size(&self) -> u64;
}

pub struct DeviceMemoryMapping {
    info: Box<dyn DeviceMemoryMappingInfo + Send>,
}
...
mod tests {
    ...
    impl DeviceMemoryMappingInfo for MemoryInfoForTest {
        fn as_ptr(&self) -> *mut u8 {
            self.hva as *mut u8
        }
        fn size(&self) -> u64 {
    ...
    #[test]
    fn device_memory_mapping_memory_invalid_address() {
        let res =
            unsafe { DeviceMemoryMapping::new(Box::new(MemoryInfoForTest { hva: 0, size: 1 })) };
        match res {
            Err(Error::InvalidAddress) => (),
            Err(e) => panic!("Unexpected error: {}", e),
            Ok(_) => {
                panic!("Should not be able to create DeviceMemoryMapping with null host address")
            }
        }
    }

    #[test]
    fn device_memory_mapping_memory_invalid_size() {
        let res =
            unsafe { DeviceMemoryMapping::new(Box::new(MemoryInfoForTest { hva: 1, size: 0 })) };
        match res {
            Err(Error::InvalidAddress) => (),
            Err(e) => panic!("Unexpected error: {}", e),
            Ok(_) => panic!("Should not be able to create DeviceMemoryMapping with zero size"),
        }
    }

    #[test]
    fn device_memory_mapping_memory_valid_parameters() {
        let res =
            unsafe { DeviceMemoryMapping::new(Box::new(MemoryInfoForTest { hva: 1, size: 1 })) };
        match res {
            Ok(_) => (),
            Err(e) => panic!("Unexpected error: {}", e),
        }
    ...
    #[test]
    fn device_memory_mapping_memory_get_address() {
        let address = 1234 as *mut u8;
        let res = unsafe {
            DeviceMemoryMapping::new(Box::new(MemoryInfoForTest {
                hva: address as u64,
                size: 1,
            }))
        };
    ...
    #[test]
    fn device_memory_mapping_memory_get_address_with_size() {
        let address = 1234 as *mut u8;
