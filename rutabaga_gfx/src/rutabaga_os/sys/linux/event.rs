...
impl Event {
    ...
    pub fn signal(&mut self) -> RutabagaResult<()> {
        self.file.write(&mut 1u64.to_ne_bytes())?;
        Ok(())
    }
...
impl Into<RutabagaHandle> for Event {
    fn into(self) -> RutabagaHandle {
        RutabagaHandle {
            // SAFETY: Safe because the file is valid and owned by us.
            os_handle: unsafe {
                SafeDescriptor::from_raw_descriptor(self.file.into_raw_descriptor())
            },
