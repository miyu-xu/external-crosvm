...
/// When attempting an allocation, each allocator will be tried in order until the allocation is successful.
/// See `AddressAllocator` for function documentation.
pub struct AddressAllocatorSet<'a> {
    allocators: Vec<&'a mut AddressAllocator>,
}

impl<'a> AddressAllocatorSet<'a> {
    pub fn new(allocators: Vec<&'a mut AddressAllocator>) -> Self {
