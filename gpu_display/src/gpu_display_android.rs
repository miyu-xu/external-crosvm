...
extern "C" {
    ...
    /// Destorys the Android surface created from `create_android_surface`.
    ...
    /// Posts the butter obtained from `get_android_surface_buffer` to the Android display system
...
impl Drop for AndroidDisplayContext {
    fn drop(&mut self) {
        ...
        unsafe { destroy_android_display_context(self) };
    }
}
...
impl From<ANativeWindow_Buffer> for GpuDisplayFramebuffer<'_> {
    fn from(anb: ANativeWindow_Buffer) -> Self {
        ...
        // SAFETY: get_android_surface_buffer guarantees that bits points to a valid buffer and the
        // buffer remains available until post_android_surface_buffer is called.
        let buffer =
...
struct AndroidSurface {
    context: Rc<NonNull<AndroidDisplayContext>>,
...
impl DisplayAndroid {
    pub fn new(name: &str) -> GpuDisplayResult<DisplayAndroid> {
        ...
        Ok(DisplayAndroid { context: context.into(), event })
    }
}
