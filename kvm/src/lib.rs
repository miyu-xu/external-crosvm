...
impl Vcpu {
    ...
    /// Gets the value of one register on this VCPU.  The id of the register is
    /// encoded as specified in the kernel documentation for KVM_GET_ONE_REG.
    #[cfg(any(target_arch = "arm", target_arch = "aarch64"))]
...
impl RunnableVcpu {
    ...
    pub fn run(&self) -> Result<VcpuExit> {
        ...
        warn!("debug request rets {}",retttt);
        let derp = self.get_one_reg(0x6030000000100040 as u64);
        match derp {
            Ok(__r) => warn!("whee {:X}", __r),
            Err(e) => warn!("meh"),
        }
        // warn!("cpu fd entering {:p}", &(self.vcpu));
