...
use resources::NonMappedDeviceMemory;
...
pub struct Vm {
    ...
    non_mapped_device_memory: HashMap<u32, NonMappedDeviceMemory>,
...
impl Vm {
    ...
    pub fn new(kvm: &Kvm, guest_mem: GuestMemory) -> Result<Vm> {
        ...
        if ret >= 0 {
            ...
            Ok(Vm {
                ...
                non_mapped_device_memory: HashMap::new(),
        ...
        let slot = match self.mem_slot_gaps.pop() {
            ...
            None => {
                (self.mmio_memory.len()
                    ...
                    + self.non_mapped_device_memory.len()) as u32
            }
        };
    ...
    /// Inserts the given `NonMappedDeviceMemory` into the VM's address space at `guest_addr`.
    ///
    /// The slot that was assigned the memory is returned on success. The slot can be given to
    /// `Vm::remove_non_mapped_device_memory` to remove the memory from the VM's address space and
    /// take back ownership of `mem`.
    ///
    /// Same rules apply for the `read_only` argument as for add_mmio_memory.  We do not currently
    /// support dirty page logging here.
    pub fn add_non_mapped_device_memory(
        &mut self,
        guest_addr: GuestAddress,
        ...
        // assume that the presence of NonMappedDeviceMemory means that it referes to a valid range
        ...
        self.non_mapped_device_memory.insert(slot, mem);

        Ok(slot)
    }

    /// Removes NonMappedDeviceMemory that was previously added at the given slot.
    pub fn remove_non_mapped_device_memory(&mut self, slot: u32) -> Result<NonMappedDeviceMemory> {
        if self.non_mapped_device_memory.contains_key(&slot) {
            // Safe because the slot is checked against the list of mmio memory slots.
            unsafe {
                self.remove_user_memory_region(slot)?;
            }
            // Safe to unwrap since map is checked to contain key
            Ok(self.non_mapped_device_memory.remove(&slot).unwrap())
        } else {
            Err(Error::new(ENOENT))
        }
    }
