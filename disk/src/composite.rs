            ...
            component_disks: self
                ...
                .map(|disk| -> crate::Result<_> {
                    Ok(AsyncComponentDiskPart {
                        ...
                        offset: disk.offset,
                        length: disk.length,
                        needs_fsync: sync::Mutex::new(disk.needs_fsync),
                    })
                })
...
/// If the regions total more than `max`, returns a vec of regions truncated to `max`.
/// If the regions total less than `max`, returns None.
fn truncate_mem_regions(max: usize, regions: &[MemRegion]) -> Option<Vec<MemRegion>> {
...
impl AsyncDisk for AsyncCompositeDiskFile {
    ...
    async fn fsync(&self) -> crate::Result<()> {
        ...
        for disk in self.component_disks.iter() {
            let should_fsync = {
                let mut needs_fsync = disk.needs_fsync.lock();
                if *needs_fsync {
                    *needs_fsync = false;
                    true
                } else {
    ...
    async fn read_to_mem<'a>(
        ...
        mut mem_offsets: &'a [MemRegion],
    ) -> crate::Result<usize> {
        let disk = self
            .disk_at_offset(file_offset)
            .map_err(crate::Error::ReadingData)?;
        let remaining_disk = disk.offset + disk.length - file_offset;
        let truncated_mem_offsets;
        if let Some(x) = truncate_mem_regions(remaining_disk.try_into().unwrap(), mem_offsets) {
            truncated_mem_offsets = x;
            mem_offsets = &truncated_mem_offsets;
        }
        disk.file
...
mod tests {
    ...
    fn async_single_file_passthrough() {
        ...
        ex.run_until(async {
            let composite = Box::new(composite).to_async_disk(&ex).unwrap();
            let expected = [55u8; 5];
            assert_eq!(composite.write(0, &expected).await.unwrap(), 5);
            let mut buf = [0u8; 5];
            assert_eq!(composite.read(0, &mut buf[..]).await.unwrap(), 5);
    ...
    fn async_triple_file_passthrough() {
        ...
        ex.run_until(async {
            ...
            assert_eq!(composite.write(0, &expected).await.unwrap(), 100);
            assert_eq!(composite.write(100, &expected[100..]).await.unwrap(), 100);

            let mut buf = [0u8; 200];
            assert_eq!(composite.read(0, &mut buf[..]).await.unwrap(), 100);
            assert_eq!(composite.read(100, &mut buf[100..]).await.unwrap(), 100);
            assert_eq!(buf, expected);
        }).unwrap();
    }
    ...
    fn async_triple_file_punch_hole() {
        ...
        ex.run_until(async {
            ...
            let input = [55u8; 300];
            assert_eq!(composite.write(0, &input).await.unwrap(), 100);
            assert_eq!(composite.write(100, &input[100..]).await.unwrap(), 100);
            assert_eq!(composite.write(200, &input[200..]).await.unwrap(), 100);
            ...
            assert_eq!(composite.read(0, &mut buf[..]).await.unwrap(), 100);
            assert_eq!(composite.read(100, &mut buf[100..]).await.unwrap(), 100);
            assert_eq!(composite.read(200, &mut buf[200..]).await.unwrap(), 100);
