...
use cdisk::{CompositeDiskFile, CDISK_MAGIC, CDISK_MAGIC_LEN};
    ...
    R: Read + Seek + SeekHole,
{
    ...
    match dst_type {
        ...
        #[cfg(feature = "composite-disk")]
        ImageType::CompositeDisk => Err(Error::ConversionNotSupported),
    }
}
...
pub fn detect_image_type(file: &File) -> Result<ImageType> {
    ...
    f.seek(SeekFrom::Start(0)).map_err(Error::SeekingFile)?;
    let magic = f.read_u32::<BigEndian>().map_err(Error::SeekingFile)?;
...
pub fn create_virtio_disk(
    ...
    Ok(match image_type {
        ImageType::Raw => {
            ...
            let dev = virtio::Block::new(raw_image, read_only, Some(disk_device_socket))
                .map_err(Error::BlockDeviceNew)?;
            Box::new(dev) as Box<dyn VirtioDevice>
        }
        ImageType::Qcow2 => {
            // Valid qcow header present
            let qcow_image = QcowFile::from(raw_image).map_err(Error::QcowError)?;
            let dev = virtio::Block::new(qcow_image, read_only, Some(disk_device_socket))
                .map_err(Error::BlockDeviceNew)?;
            Box::new(dev) as Box<dyn VirtioDevice>
        }
        #[cfg(feature = "composite-disk")]
        ImageType::CompositeDisk => {
            ...
            let cdisk_image =
                CompositeDiskFile::from_file(raw_image).map_err(Error::CreateCompositeDisk)?;
            let dev = virtio::Block::new(cdisk_image, read_only, Some(disk_device_socket))
                .map_err(Error::BlockDeviceNew)?;
            Box::new(dev) as Box<dyn VirtioDevice>
        }
    })
