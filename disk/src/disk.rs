...
pub fn create_disk_file(params: DiskFileParams) -> Result<Box<dyn DiskFile>> {
    ...
    Ok(match image_type {
        ImageType::Raw => {
            sys::apply_raw_disk_file_options(&params.raw_image, params.is_sparse_file)?;
            Box::new(params.raw_image) as Box<dyn DiskFile>
        }
        #[cfg(feature = "qcow")]
        ImageType::Qcow2 => {
            Box::new(QcowFile::from(params).map_err(Error::QcowError)?) as Box<dyn DiskFile>
        }
        #[cfg(feature = "composite-disk")]
        ImageType::CompositeDisk => {
            // Valid composite disk header present
            Box::new(CompositeDiskFile::from_file(params).map_err(Error::CreateCompositeDisk)?)
                as Box<dyn DiskFile>
        }
        #[cfg(feature = "android-sparse")]
        ImageType::AndroidSparse => Box::new(
            AndroidSparse::from_file(params.raw_image).map_err(Error::CreateAndroidSparseDisk)?,
        ) as Box<dyn DiskFile>,
