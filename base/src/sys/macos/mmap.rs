...
/// Largest file offset accepted for `mmap(2)` on macOS (`off_t`).
const MAX_MMAP_OFFSET: u64 = libc::off_t::MAX as u64;

#[inline]
unsafe fn mmap_with_offset(
...
impl MemoryMapping {
    ...
    /// Helper wrapper around `mmap(2)` with basic validation (Darwin-specific behavior).
    unsafe fn try_mmap_populate(
        ...
        // `populate` is accepted for API parity with Linux; Darwin has no MAP_POPULATE.
        let _ = populate;
        ...
        let (addr, orig_addr, orig_size) = match align {
            ...
            Some(align) => {
                ...
                let orig_addr = mmap_with_offset(
                    null_mut(),
                    orig_size,
                    prot,
        ...
        let addr = mmap_with_offset(addr, size, prot, flags, fd, offset);
    ...
    pub fn use_dontfork(&self) -> Result<()> {
        // MADV_DONTFORK is Linux-specific; not available on Darwin.
        let _ = self;
        Ok(())
    }
    ...
    pub fn use_hugepages(&self) -> Result<()> {
        ...
        // Transparent huge pages / MADV_HUGEPAGE are Linux-specific.
        Ok(())
    }
    ...
    pub fn lock_on_fault(&self, mem_offset: usize, count: usize) -> Result<()> {
        ...
        // Darwin does not expose mlock2(MLOCK_ONFAULT); fall back to mlock(2).
        let ret = unsafe { libc::mlock(addr as *mut libc::c_void, count) };
        if ret < 0 {
...
mod tests {
    ...
    fn arena_madvise() {
        ...
        m.add_anon_protection(0, size, Protection::read_write())
            .expect("failed to add writable protection for madvise");
        let ps = pagesize();
        <dyn MappedRegion>::madvise(&m, 0, ps, libc::MADV_FREE).unwrap();
        <dyn MappedRegion>::madvise(&m, 0, size, libc::MADV_FREE).unwrap();
        <dyn MappedRegion>::madvise(&m, ps, size - ps, libc::MADV_WILLNEED).unwrap();
