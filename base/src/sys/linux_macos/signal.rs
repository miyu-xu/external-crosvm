...
#[cfg(target_os = "macos")]
const SIGPWR_COMPAT: c_int = libc::SIGINFO;
#[cfg(not(target_os = "macos"))]
const SIGPWR_COMPAT: c_int = libc::SIGPWR;

#[cfg(target_os = "macos")]
const SIGSTKFLT_COMPAT: c_int = libc::SIGEMT;
#[cfg(not(target_os = "macos"))]
const SIGSTKFLT_COMPAT: c_int = libc::SIGSTKFLT;
...
// macOS does not expose glibc-style RT signals; crosvm only needs a dedicated async signal slot
// (see `vcpu.rs`: `SIGRTMIN() + 0`). Map the RT range to SIGUSR1/SIGUSR2.
...
pub fn wait_for_signal(signals: &[c_int], timeout: Option<Duration>) -> Result<c_int> {
    ...
    match timeout {
        Some(_timeout) => {
            ...
            {
                let _ = sigset;
                Err(ErrnoError::new(libc::ENOSYS))
            }
            #[cfg(not(target_os = "macos"))]
            {
                let ts = duration_to_timespec(_timeout);
                // SAFETY:
                // Safe - return value is checked.
