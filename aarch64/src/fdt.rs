...
fn create_trusty_node(fdt: &mut FdtWriter) -> Result<()> {
    ...
    // create a log node
    let trusty_log_node = fdt.begin_node("log")?;
    fdt.property_string("compatible", "android,trusty-log-v1")?;
    fdt.end_node(trusty_log_node)?;

    // TODO: create an irq node
    // TODO: create a test node
