...
use base::sys::platform::sched_attr;
use base::warn;
use base::Error;
use std::io::Write;
...
impl BusDevice for VirtCpufreq {
    ...
    fn write(&mut self, _info: BusAccessInfo, data: &[u8]) {
        ...
        if let Some(socket) = &self.socket {
            let tid = base::linux::gettid();
            let msg = (util as u32) << 32 | (tid as u32);
            match socket.lock().write_all(msg.as_bytes()) {
                Ok(_) => (), // Ignore result on read success
                Err(e) => {
                    warn!(
                        "{}: socket write failed! error:{}",
                        self.debug_label(),
                        e
                    );
                    return;
