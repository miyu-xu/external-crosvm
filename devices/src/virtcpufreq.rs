...
impl Suspendable for VirtCpufreq {
    ...
    fn snapshot(&mut self) -> anyhow::Result<serde_json::Value> {
        serde_json::to_value(&self).with_context(|| format!("failed to serialize {}", self.debug_label()))
    }

    fn restore(&mut self, data: serde_json::Value) -> anyhow::Result<()> {
        let deser: Self = serde_json::from_value(data).with_context(|| format!("failed to deserialize {}", self.debug_label()))?;
        self.cpu_fmax = deser.cpu_fmax;
        self.cpu_capacity = deser.cpu_capacity;
        self.pcpu = deser.pcpu;
        self.util_factor = deser.util_factor;
        Ok(())
    }
}
