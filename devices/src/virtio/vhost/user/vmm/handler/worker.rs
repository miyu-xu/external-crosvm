...
impl Worker {
    ...
    pub fn run(&mut self, interrupt: Interrupt) -> Result<(), String> {
        ...
        let handle_non_msix_evt = handle_non_msix_evt(&ex, &self.non_msix_evt, interrupt.clone());
...
async fn handle_non_msix_evt(
    ...
    let event = non_msix_evt
        .try_clone()
        .expect("failed to clone non_msix_evt");
    let event_async = EventAsync::new(event, ex).expect("failed to create async non_msix_evt");
    loop {
