...
impl VirtioDevice for Console {
    ...
    fn virtio_sleep(&mut self) -> anyhow::Result<Option<BTreeMap<usize, Queue>>> {
        self.console.stop_input_threads();

        // Stop and collect all the queues.
        let mut queues = BTreeMap::new();
        for idx in 0..self.console.max_queues() {
            if let Ok(queue) = self.console.stop_queue(idx) {
...
mod tests {
    ...
    fn test_inactive_sleep_resume() {
        ...
        // Make sure the input and output are returned to the port during sleep.
        let sleep_result = device.virtio_sleep().expect("failed to sleep");
        assert!(sleep_result.is_none());
        assert!(device.console.ports[0].input.is_some());
        assert!(device.console.ports[0].output.is_some());

        // Ensure snapshot does not fail.
        let _snapshot = device.virtio_snapshot().expect("failed to snapshot");

        // Wake up the device, which should start the input thread again.
        device.virtio_wake(None).expect("failed to wake");

        // Put the device to sleep again.
        let _sleep_result2 = device.virtio_sleep().expect("failed to sleep");
        assert!(device.console.ports[0].input.is_some());
        assert!(device.console.ports[0].output.is_some());
    }
}
