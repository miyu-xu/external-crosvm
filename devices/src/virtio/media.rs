...
///
/// This is required by the fact that devices can be instantiated several times, but the mapper is
...
impl<M: SharedMemoryMapper> VirtioMediaHostMemoryMapper for HostMemoryMapper<M> {
    fn add_mapping(
        ...
        // TODO technically `offset` can be used twice if a buffer is removed and re-added.
        let shm_offset = self
            .allocator
...
impl AsRef<[u8]> for GuestMemoryMapping {
    fn as_ref(&self) -> &[u8] {
        // Safe because the arena has a valid pointer that covers the slice we are creating.
        unsafe {
...
/// Copy of sparse guest memory that is written back upon description.
///
...
/// the size of the guest data. If it is below `MAPPING_THRESHOLD`, a shadow buffer is used ;
...
/// Worker to operate a virtio-media devices inside a worker thread.
    ...
    D: VirtioMediaDevice<Reader, Writer>,
{
    ...
    fn new(
        ...
        wait_ctx
            ...
            .context("wait context creation failed")?;
        ...
        loop {
            ...
            for wait_event in wait_events.iter() {
                match wait_event.token {
                    ...
                    Token::V4l2Session(session_id) => {
                        let session = self.runner.sessions.get_mut(&session_id).context(
                            format!("received event for non-registered session {}", session_id),
                        )?;
...
pub fn create_virtio_media_v4l2_proxy_device<P: AsRef<Path>>(
    ...
    let device = CrosvmVirtioMediaDevice::new(
        ...
        move |event_queue, guest_mapper, host_mapper| {
            let device = virtio_media::devices::V4l2ProxyDevice::new(
                device_path.clone(),
                event_queue,
                guest_mapper,
                host_mapper,
...
#[cfg(all(feature = "video-decoder", feature = "ffmpeg"))]
pub fn create_virtio_media_decoder_adapter_device(
    features: u64,
    _gpu_tube: Tube,
) -> Box<dyn VirtioDevice> {
    use crate::virtio::media::decoder_adapter::VirtioMediaDecoderAdapter;
    use crate::virtio::video::decoder::backend::ffmpeg::FfmpegDecoder;
    use virtio_media::v4l2r::ioctl::Capabilities;

    let mut card = [0u8; 32];
    let card_name = "virtio_decoder";
    card[0..card_name.len()].copy_from_slice(card_name.as_bytes());
    ...
    let create_device =
        |event_queue, _, host_mapper: HostMemoryMapper<ArcedMemoryMapper>, wait_context: &_| {
            let backend = FfmpegDecoder::new();
            //let decoder = Decoder::new(backend, video_tube, guest_mapper.0.clone());
            let adapter_device = VirtioMediaDecoderAdapter::new(
                backend,
                event_queue,
                host_mapper,
                Arc::clone(wait_context),
            );
    ...
    let device =
        CrosvmVirtioMediaDevice::new(features, config, create_device, |session_id, _, worker| {
            let session = worker
                .runner
                .sessions
                .get_mut(&session_id)
            ...
            worker.runner.device.process_one_event(session);
