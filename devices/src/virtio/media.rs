...
impl GuestMemoryMapping {
    fn new(mem: &GuestMemory, sgs: &[SgEntry]) -> anyhow::Result<Self> {
        ...
        let arena_size = sgs.iter().fold(0, |size, sg| {
            let sg_full_size = (sg.start & page_mask) + sg.len as u64;
            size + sg_full_size.next_multiple_of(page_size)
        });
        ...
        for region in sgs {
            ...
            let region_page_start = region.start & !page_mask;
            let len = region.len as u64 + region.start - region_page_start;
        ...
        + Send,
{
    ...
    fn activate(
        ...
        let worker_handle = thread::Builder::new()
            .name("vmedia_worker".to_owned())
            .spawn(move || {
                let wait_ctx = Rc::new(wait_ctx);
                let mut worker = match Worker::new(device, cmd_queue, interrupt, kill_evt, wait_ctx)
                {
                    Ok(worker) => worker,
                    Err(e) => {
            ...
            .context("virtio-media worker thread exitted unexpectedly")?;
