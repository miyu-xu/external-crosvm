...
impl VioSClient {
    ...
    pub fn request_audio_data(
        ...
        let (src_offset, status_promise) = {
            ...
            let rx = &mut *rx_lock;
            let src_offset = rx.allocate_buffer(dst_offset, size)?;
...
impl Drop for VioSClient {
    fn drop(&mut self) {
        ...
        *self.recv_running.lock() = false;
        self.recv_event.lock().write(1u64);
                ...
                match evt.token {
                    Token::RxBufferMsg => {
                        ...
                        match promise_opt {
                            ...
                            Some(sender) => {
                                sender.send((status, consumed_len));
                            }
                        }
                    }
                    Token::Notification => {
...
impl IoBufferQueue {
    ...
    fn allocate_buffer(&mut self, offset: usize, size: usize) -> Result<usize> {
        if size > self.size {
    ...
    fn push_buffer(&mut self, src: &mut SharedMemory, offset: usize, size: usize) -> Result<usize> {
        let shm_offset = self.allocate_buffer(offset, size)?;
