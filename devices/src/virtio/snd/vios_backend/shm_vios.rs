...
pub struct VioSClient {
    ...
    tx_subscribers: Arc<Mutex<HashMap<usize, Sender<(u32, u32, usize)>>>>,
    rx_subscribers: Arc<Mutex<HashMap<usize, Sender<(u32, u32, usize)>>>>,
...
impl VioSClient {
    ...
    pub fn try_new<P: AsRef<Path>>(server: P) -> Result<VioSClient> {
        ...
        let tx_subscribers: Arc<Mutex<HashMap<usize, Sender<(u32, u32, usize)>>>> =
            Arc::new(Mutex::new(HashMap::new()));
        let rx_subscribers: Arc<Mutex<HashMap<usize, Sender<(u32, u32, usize)>>>> =
            Arc::new(Mutex::new(HashMap::new()));
    ...
    pub fn jack_info(&self, idx: u32) -> Option<virtio_snd_jack_info> {
        self.jacks.lock().get(idx as usize).map(|rf| rf.clone())
    }
    ...
    pub fn stream_info(&self, idx: u32) -> Option<virtio_snd_pcm_info> {
        self.streams
            .lock()
            .get(idx as usize)
            .map(|info| virtio_snd_pcm_info::from(info))
    }

    pub fn chmap_info(&self, idx: u32) -> Option<virtio_snd_chmap_info> {
        self.chmaps.lock().get(idx as usize).map(|rf| rf.clone())
    }
    ...
    pub fn inject_audio_data<R, Cb: FnOnce(VolatileSlice) -> R>(
        ...
        let (status_promise, ret) = {
            ...
            let (sender, receiver): (Sender<(u32, u32, usize)>, Receiver<(u32, u32, usize)>) =
                channel();
        ...
        self.jacks = Mutex::new(
            info_vec
                ...
                .map(|info_buffer| {
                    ...
                    virtio_snd_jack_info::from_slice(&info_buffer)
                        .unwrap()
                        .clone()
                })
                .collect(),
    ...
    fn request_and_cache_chmaps_info(&mut self) -> Result<()> {
        ...
        self.chmaps = Mutex::new(
            info_vec
                ...
                .map(|info_buffer| {
                    // unwrap is safe because we checked the size of the vector
                    virtio_snd_chmap_info::from_slice(&info_buffer)
                        .unwrap()
                        .clone()
                })
                .collect(),
...
fn recv_buffer_status_msg(
    ...
    subscribers: &Arc<Mutex<HashMap<usize, Sender<(u32, u32, usize)>>>>,
) -> Result<()> {
    ...
    match promise_opt {
        ...
        Some(sender) => {
            if let Err(e) = sender.send((status, latency, consumed_len)) {
...
fn await_status(promise: Receiver<(u32, u32, usize)>) -> Result<(usize, u32)> {
    let (status, latency, consumed_len) = promise.recv().map_err(Error::BufferStatusSenderLost)?;
