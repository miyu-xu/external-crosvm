...
pub struct Worker {
    // Lock order: Any queue lock must be acquired before the interrupt lock. Must never hold
    // more than one queue lock at the same time.
...
impl Worker {
    ...
    pub fn try_new(
        ...
        let mut streams: Vec<StreamProxy> = Vec::new();
        {
    ...
    pub fn control_loop(&mut self, kill_evt: Event) -> Result<()> {
        ...
        if let Some(resample_evt) = self.interrupt.lock().get_resample_evt() {
            wait_ctx
    ...
    fn process_controlq_buffers(&mut self) -> Result<()> {
        while let Some(avail_desc) = lock_pop_unlock(&self.control_queue, &self.guest_memory) {
            ...
            if request_type == VIRTIO_SND_R_JACK_INFO {
                let (code, info_vec) = {
                    match self.parse_info_query(&read_buf) {
                        None => (VIRTIO_SND_S_BAD_MSG, Vec::new()),
                        Some((start_id, count)) => {
                            if start_id + count > self.vios_client.num_jacks() {
                                error!(
            ...
            } else if request_type == VIRTIO_SND_R_JACK_REMAP {
                    ...
                    .map_err(SoundError::QueueIO)?;
                {
                    ...
                    queue_lock.trigger_interrupt(&self.guest_memory, interrupt_lock.deref());
                }
            } else if request_type == VIRTIO_SND_R_CHMAP_INFO {
                let (code, info_vec) = {
                    match self.parse_info_query(&read_buf) {
                        None => (VIRTIO_SND_S_BAD_MSG, Vec::new()),
                        Some((start_id, count)) => {
                            if start_id + count > self.vios_client.num_chmaps() {
                                error!(
                ...
                self.send_info_reply(avail_desc, code, info_vec)?;
            } else if request_type == VIRTIO_SND_R_PCM_INFO {
                let (code, info_vec) = {
                    match self.parse_info_query(&read_buf) {
                        None => (VIRTIO_SND_S_BAD_MSG, Vec::new()),
                        Some((start_id, count)) => {
                            if start_id + count > self.vios_client.num_streams() {
                                error!(
                ...
                match request_type {
                    VIRTIO_SND_R_PCM_SET_PARAMS => {
                        self.process_set_params(avail_desc, &read_buf)?
                    }
                    VIRTIO_SND_R_PCM_PREPARE => self.try_parse_pcm_hdr_and_send_msg(
                        &read_buf,
                        StreamMsg::Prepare(avail_desc),
                    )?,
                    VIRTIO_SND_R_PCM_RELEASE => self.try_parse_pcm_hdr_and_send_msg(
                        &read_buf,
                        StreamMsg::Release(avail_desc),
                    )?,
                    VIRTIO_SND_R_PCM_START => self
                        .try_parse_pcm_hdr_and_send_msg(&read_buf, StreamMsg::Start(avail_desc))?,
