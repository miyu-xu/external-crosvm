...
fn process_control_msg(
    reader: &mut Reader,
    ports: &[WorkerPort],
    control_replies: &mut VecDeque<ControlMsgBytes>,
) -> anyhow::Result<()> {
    ...
    match event {
        VIRTIO_CONSOLE_DEVICE_READY => {
            ...
            for (index, port) in ports.iter().enumerate() {
                ...
                // TODO(dverkamp): cap the size of `control_replies` somehow
                control_replies.push_back(control_msg(port_id, VIRTIO_CONSOLE_DEVICE_ADD, 0, &[]));

                if let Some(name) = port.name() {
                    control_replies.push_back(control_msg(
                        port_id,
        ...
        VIRTIO_CONSOLE_PORT_READY => {
            // value of 1 indicates success, and 0 indicates failure
            if value != 1 {
                return Err(anyhow!("console port ready failure ({value})"));
            }
            ...
            control_replies.push_back(control_msg(id, VIRTIO_CONSOLE_PORT_OPEN, 1, &[]));

            if port.is_console() {
                control_replies.push_back(control_msg(id, VIRTIO_CONSOLE_CONSOLE_PORT, 1, &[]));
            }
            Ok(())
        }
...
pub fn process_control_transmit_queue(
    ...
    while let Some(mut avail_desc) = queue.pop() {
        if let Err(e) = process_control_msg(&mut avail_desc.reader, ports, control_replies) {
...
pub fn process_control_receive_queue(
    ...
    while !control_replies.is_empty() {
        let Some(mut avail_desc) = queue.pop() else {
            break;
        };

        // Get a reply to copy into `avail_desc`. This should never fail since we check that
        // `control_replies` is not empty in the loop condition.
        let reply = control_replies.pop_front().expect("missing reply");

        if let Err(e) = avail_desc.writer.write_all(&reply) {
