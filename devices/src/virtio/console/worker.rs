...
impl WorkerPort {
    pub fn from_console_port(port: &mut ConsolePort) -> WorkerPort {
        ...
        let info = port.port_info().map(|info| (*info).clone());
        WorkerPort {
            info,
            in_avail_evt,
            input_buffer,
...
pub enum WorkerRequest {
    ...
    StopQueue {
        idx: usize,
        response_sender: mpsc::SyncSender<anyhow::Result<Queue>>,
    },
}
...
pub struct Worker {
    ...
    control_replies: VecDeque<ControlMsgBytes>,
...
impl Worker {
    ...
    fn stop_queue(&mut self, idx: usize) -> anyhow::Result<Queue> {
        if let Some(queue) = self.queues.remove(&idx) {
            ...
            Err(anyhow!("queue {idx} not started"))
        }
    }
}
...
impl WorkerHandle {
    pub fn new(interrupt: Interrupt, ports: Vec<WorkerPort>) -> anyhow::Result<Self> {
        ...
        let worker_thread = WorkerThread::start("v_console", move |kill_evt| {
            match Worker::new(interrupt, ports, worker_receiver, worker_event_clone) {
                Ok(mut worker) => {
                ...
                Err(e) => {
                    error!("console Worker::new() failed: {:#}", e);
                }
            }
        });
        Ok(WorkerHandle {
