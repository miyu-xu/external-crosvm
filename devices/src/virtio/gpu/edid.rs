...
impl EdidBytes {
    ...
    pub fn new(info: &DisplayInfo) -> VirtioGpuResult {
        ...
        let mut block0 = &mut edid[54..72];
        populate_detailed_timing(block0, info);

        let mut block1 = &mut edid[72..90];
...
fn populate_display_name(edid_block: &mut [u8]) {
    ...
    edid_block[3] = 0xFC;
    edid_block[4] = 0x00;
    for (i, c) in "CrosvmDisplay".chars().enumerate() {
        edid_block[5 + i] = get_ascii_byte(c);
    }
}
...
fn populate_header(edid: &mut [u8]) {
    ...
    // GOOGLE -> GGL
    let manufacturer_id_letter_0: u16 = ('G' as u16) - 64;
    let manufacturer_id_letter_1: u16 = ('G' as u16) - 64;
    let manufacturer_id_letter_2: u16 = ('L' as u16) - 64;

    let manufacturer_id_letter_0_lsb: u16 = manufacturer_id_letter_0 & 0x1F; // least sig 5 bits
    let manufacturer_id_letter_1_lsb: u16 = manufacturer_id_letter_1 & 0x1F; // least sig 5 bits
    let manufacturer_id_letter_2_lsb: u16 = manufacturer_id_letter_2 & 0x1F; // least sig 5 bits

    let manufacture_id: u16 = (manufacturer_id_letter_2_lsb << 0)
        | (manufacturer_id_letter_1_lsb << 5)
        | (manufacturer_id_letter_0_lsb << 10);
    edid[8..10].copy_from_slice(&manufacture_id.to_be_bytes());
