...
pub struct VirtioGpu {
    ...
    // Maps virtio gpu's scanout id to the current resource set.
...
impl VirtioGpu {
    ...
    pub fn new(
        ...
        error!("jasonjason VirtioGpu::new()");
    ...
    /// Returns the displays that need to be updated when the given resource is flushed.
    fn get_scanout_ids_for_resource(&self, resource_id: u32) -> Vec<u32> {
    ...
    pub fn get_or_create_scanout_surface_id(&mut self, scanout_id: u32) -> Option<u32> {
        ...
        match self.scanout_id_to_surface_id.entry(scanout_id) {
            ...
            Entry::Vacant(entry) => {
                if let Some(display_params) = self.display_params.lock().displays.get(scanout_id as usize) {
    ...
    pub fn get_scanout_dimensions(&self, scanout_id: u32) -> Option<(u32, u32)> {
        self.display_params.lock().displays.get(scanout_id as usize).map(|params| (params.width, params.height))
    }
    ...
    pub fn display_info(&self) -> Vec<(u32, u32)> {
        self.display_params.lock().get_display_dimension_pairs()
    }

    pub fn add_display(&mut self, width: u32, height: u32) -> DisplayControlResult {
        self.display_params.lock().add_display(width, height)
    }

    pub fn list_displays(&self) -> DisplayControlResult {
        self.display_params.lock().list_displays()
    }

    pub fn remove_display(&mut self, display_id: u32) -> DisplayControlResult {
        self.display_params.lock().remove_display(display_id)
    }
    ...
    pub fn process_display(&mut self) -> bool {
        ...
        let closed = self.scanout_id_to_surface_id.values().any(|surface_id| display.close_requested(*surface_id));
        if closed {
            error!("jasonjason display closed!");
        }
        closed
    }
    ...
    pub fn set_scanout(
        ...
        error!("jasonjason set_scanout(scanout_id: {:?}, resource_id: {:?})", scanout_id, resource_id);
    ...
    pub fn flush_resource(&mut self, resource_id: u32) -> VirtioGpuResult {
        error!("jasonjason flush_resource(resource_id: {:?})", resource_id);
        ...
        let scanout_ids = self.get_scanout_ids_for_resource(resource_id);
        for scanout_id in scanout_ids {
            ...
            if let Some(surface_id) = self.scanout_id_to_surface_id.get(&scanout_id) {
                response = self.flush_resource_to_surface(resource_id,
                                                          *surface_id,
                                                          surface_width,
                                                          surface_height);
                if !response.is_ok() {
    ...
    pub fn import_to_display(&mut self, resource_id: u32) -> Option<u32> {
        error!("jasonjason import_to_display(resource_id: {:?})", resource_id);
    ...
    pub fn update_cursor(&mut self, resource_id: u32, scanout_id: u32, x: u32, y: u32) -> VirtioGpuResult {
        ...
        let scanout_surface_id = match self.get_or_create_scanout_surface_id(scanout_id) {
            Some(id) => id,
            None => {
