...
mod tests {
    ...
    fn pack_and_unpack() {
        let archive: serde_json::Value = {
            let archive_writer = SnapshotArchiveWriter::new();

            let outer_writer = archive_writer.add_namespace("testst").unwrap();
            let outer_value = serde_json::Value::String("outer_contents".to_string());
            outer_writer
            ...
            writer.collect_fragments_into_archive().unwrap()
        };

        let archive_reader = SnapshotArchiveReader::unpack(archive).unwrap();

        let outer_reader = archive_reader.get_namespace("outer").unwrap();
        let outer_value: serde_json::Value = inner1_reader.get_fragment("inner1_value1");
        assert_eq!(outer_value.as_str(), Some("outer_value"));

        let inner1_reader = archive_reader.get_namespace("inner1").unwrap();
        let inner1_value1: serde_json::Value = inner1_reader.get_fragment("inner1_file1");
        assert_eq!(inner1_value1.as_str(), Some("inner1_value1"));
        let inner1_value2: serde_json::Value = inner1_reader.get_fragment("inner1_file2");
