...
impl SnapshotArchiveWriter {
    ...
    pub fn collect_fragments_into_archive(&self) -> anyhow::Result<serde_json::Value> {
        ...
        let snapshot_files = get_files_under(snapshot_root)
        .with_context(|| format!("failed to list snapshot files under {}", snapshot_root.display()))?;

        for path in snapshot_files.into_iter() {
            let contents: Vec<u8> = std::fs::read(&path)
                .with_context(|| format!("failed to read snapshot file {}", path.display()))?;
            let contents_str = base64::encode(contents);
            let contents_json = serde_json::Value::String(contents_str);

            let relative_path = path.strip_prefix(snapshot_root)
                .with_context(|| format!("failed to strip {} from {}", snapshot_root.display(), path.display()))?
                .to_string_lossy()
                .to_string();
...
impl SnapshotArchiveReader {
    pub fn unpack(mut archive: serde_json::Value) -> anyhow::Result<Self> {
        ...
        for (path, contents_json) in archive.as_object_mut()
            ...
            let contents: Vec<u8> = base64::decode(contents_str)
...
impl Drop for SnapshotArchiveReader {
    fn drop(&mut self) {
        if let Some(dir) = self.dir.take() {
            ...
            println!("Preserving snapshot archive for debugging at {}", path.display());
        };
    }
}
