...
impl GpuResponse {
    ...
    pub fn encode(
        ...
        let len = match *self {
            ...
            GpuResponse::OkEdid(ref data) => {
                let mut edid_info = virtio_gpu_resp_edid {
                    hdr,
                    size: Le32::from(1024/*data.len() as u32*/),
                    padding: Le32::from(0),
                    edid: [0; 1024],
                };
                edid_info.edid[0..data.len() as usize].copy_from_slice(data);
                resp.get_ref(0)?.store(edid_info);
                size_of::<virtio_gpu_resp_edid>()
            }
