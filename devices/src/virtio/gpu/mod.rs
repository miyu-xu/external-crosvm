...
impl Worker {
    ...
    fn run(&mut self) -> anyhow::Result<()> {
        loop {
            let request = self
                .request_receiver
                .recv()
                .expect("failed to receive gpu worker request");
            match request {
                WorkerRequest::Resume(mut resume) => {
                    let (response, had_error) = match self.on_resume(&mut resume) {
                        Ok(()) => (WorkerResponse::Ok, false),
                        Err(e) => (WorkerResponse::Err(e), true),
                    };
                    self.response_sender
                        .send(response)
                        .expect("failed to send gpu worker response for resume");
                    ...
                    let stop_reason = self
                        ...
                        .expect("failed to run gpu worker processig");
                ...
                WorkerRequest::Suspend => {
                    let response = match self.on_suspend() {
                        Ok(deactivation_resources) => {
                            WorkerResponse::Suspend(deactivation_resources)
                        }
                ...
                WorkerRequest::Snapshot => {
                    let response = match self.on_snapshot() {
                        Ok(snapshot) => WorkerResponse::Snapshot(snapshot),
                        Err(e) => WorkerResponse::Err(e),
                    };
                    self.response_sender
                        .send(response)
                ...
                WorkerRequest::Restore(restore) => {
                    let response = match self.on_restore(restore.snapshot) {
    ...
    fn on_resume(&mut self, resume: &mut WorkerResumeRequest) -> anyhow::Result<()> {
        let activation_resources = resume
            .resources
            .take()
            .expect("gpu worker failed to resume due to missing resources");
        ...
        self.state
            ...
            .context("gpu worker failed to resume virtio frontend")?;
...
impl VirtioDevice for Gpu {
    ...
    fn activate(
        ...
        self.worker_request_sender
            ...
            .expect("failed to send virtio gpu worker resume request");
        ...
        match response {
            ...
            _ => {
                panic!("unexpected response from virtio gpu worker resume request");
            }
        }
    }
