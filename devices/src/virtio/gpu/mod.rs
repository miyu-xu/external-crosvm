...
impl Frontend {
    ...
    pub fn add_display(&mut self, width: u32, height: u32) -> GpuControlResult {
        self.virtio_gpu.add_display(width, height)
    }

    pub fn list_displays(&self) -> GpuControlResult {
        self.virtio_gpu.list_displays()
    }

    pub fn remove_display(&mut self, display_id: u32) -> GpuControlResult {
...
impl Worker {
    fn run(&mut self) {
        let mut wait_ctx: WaitContext<WorkerToken> = match WaitContext::build_with(&[
            ...
            (self.display_control_tube.as_ref().unwrap(), WorkerToken::GpuControl),
            ...
            for event in events.iter().filter(|e| e.is_readable) {
                match event.token {
                    ...
                    WorkerToken::GpuControl => {
                        let control_socket = match self.gpu_control_tube.as_ref() {
                            Some(cs) => cs,
                            None => {
                                error!(
                                    "gpu control socket request with no gpu control socket"
                                );
                                break 'wait;
                            }
                        };
                        ...
                        let resp = match req {
                            GpuControlCommand::AddDisplay { width, height } => {
                                ...
                                if resp == GpuControlResult::Ok {
                                    needs_config_interrupt = true;
                                }
                                resp
                            }
                            GpuControlCommand::ListDisplays => self.state.list_displays(),
...
impl VirtioDevice for Gpu {
    fn keep_rds(&self) -> Vec<RawDescriptor> {
        ...
        if let Some(ref gpu_control_tube) = &self.gpu_control_tube {
    ...
    fn activate(
        ...
        let gpu_control_tube = match self.gpu_control_tube.try_clone() {
            Ok(e) => e,
            Err(e) => {
                error!("error cloning exit tube: {}", e);
                return;
