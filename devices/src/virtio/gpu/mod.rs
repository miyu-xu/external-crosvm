...
impl Worker {
        ...
        loop {
            ...
            match request {
                ...
                WorkerRequest::Snapshot(snapshot_writer) => {
                    let response = self.on_snapshot(snapshot_writer).map(|_| WorkerResponse::Ok);
                    self.response_sender
                        .send(response)
    ...
    fn on_snapshot(&mut self, writer: RutabagaSnapshotWriter) -> anyhow::Result<()> {
        ...
        writer.add_fragment("fence_state", &fence_state_snapshot)
              .context("failed to write virtio gpu fence state snapshot fragment")?;

        self.state
            .virtio_gpu
            .snapshot(writer.add_namespace("virtio_gpu")?)
            .context("failed to snapshot VirtioGpu")
    }

    fn on_restore(&mut self, reader: RutabagaSnapshotReader) -> anyhow::Result<()> {
        let fence_state_snapshot: FenceStateSnapshot = reader.get_fragment("fence_state")
            .context("failed to read virtio gpu fence state snapshot fragment")?;
        self.fence_state
            .lock()
            .restore(fence_state_snapshot);

        self.state
            .virtio_gpu
            .restore(reader.get_namespace("virtio_gpu")?)
...
impl VirtioDevice for Gpu {
    ...
    fn virtio_snapshot(&mut self) -> anyhow::Result<serde_json::Value> {
            ...
            (&self.worker_request_sender, &self.worker_response_receiver)
        {
            ...
            Ok(snapshot_archiver.collect_fragments_into_archive()
