...
impl VirtioBackend {
    ...
    pub fn display_info(&self) -> Vec<(u32, u32)> {
        let display_infos = self.display_params.iter().map(|params| (params.width, params.height)).collect::<Vec<_>>();

        for (w, h) in display_infos.iter() {
            error!("jasonjason display info w:{} h:{}", w, h);
        }
        display_infos
    }
