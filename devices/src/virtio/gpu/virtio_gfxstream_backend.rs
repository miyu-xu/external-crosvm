...
impl Backend for VirtioGfxStreamBackend {
    ...
    // transfer* ioctls don't provide fence fd in/out,
    // so gfxstream actually needs this for fence fds
    fn submit_command(&mut self, ctx_id: u32, commands: &mut [u8]) -> GpuResponse {

        if commands.len() % std::mem::size_of::<u32>() != 0 {
            error!("context {} got command with size {} which is not u32 multiple", ctx_id, commands.len());
            return GpuResponse::ErrUnspec;
        }
