...
impl<D: DecoderBackend> VirtioVideoAdapterSession<D> {
    ...
    fn try_send_pending_output_buffers(&mut self) {
        ...
        // TODO: actually we should create the session with the current parameters if it doesn't
        // exist...
        ...
        if let Some(backend_session) = &mut self.backend_session {
            ...
            match event {
                DecoderEvent::NotifyEndOfBitstreamBuffer(id) => {
                    Some(VideoDecoderBackendEvent::InputBufferDone(id))
                }
                DecoderEvent::ProvidePictureBuffers {
                    ...
                    // We can queue pending CAPTURE buffers now.
    ...
    fn adjust_format(
        ...
        let (colorspace, xfer_func, ycbcr_enc, quantization) =
            if direction == QueueDirection::Output {
                (
                    Colorspace::n(pix_mp.colorspace).unwrap_or(session.colorspace),
                    XferFunc::n(pix_mp.xfer_func as u32).unwrap_or(session.xfer_func),
                    // TODO: safe because...
                    YCbCrEncoding::n(unsafe { pix_mp.__bindgen_anon_1.ycbcr_enc as u32 })
                        .unwrap_or(session.ycbcr_enc),
                    Quantization::n(pix_mp.quantization as u32).unwrap_or(session.quantization),
                )
            } else {
                (
                    session.colorspace,
    ...
    fn apply_format(
        ...
        match direction {
            QueueDirection::Output => {
                session.input_format = Format::try_from(format.pixelformat()).unwrap();
                ...
                /*
                // TODO: we should only update this if new size if smaller, rectangle is set differently.
                session.visible_rect =
                    v4l2r::Rect::new(0, 0, session.coded_size.0, session.coded_size.1);
                */
                session.colorspace = format.colorspace();
                session.xfer_func = format.xfer_func();
                session.ycbcr_enc = format.ycbcr_enc();
                session.quantization = format.quantization();
            ...
            QueueDirection::Capture => {
                session.output_format = Format::try_from(format.pixelformat()).unwrap();
