...
/// to handle the case of snapshot being potentially too large to send across a Tube in a single message.
...
impl SnapshotFile {
    fn new() -> anyhow::Result<SnapshotFile> {
        Ok(SnapshotFile{
            file: tempfile::tempfile()
                .context("failed to create snasphot wrapper tempfile")?
        })
    }
    ...
    fn read(&mut self) -> anyhow::Result<serde_json::Value> {
...

        {
            ...
            data = Some(serde_json::from_reader(&mut reader)
    ...
    loop {
        ...
        let res = match cmd {
            ...
            Command::Restore { mut snapshot } => {
                let res = snapshot.read()
                                  .and_then(|data| device.restore(data));
...
impl Suspendable for ProxyDevice {
    fn snapshot(&mut self) -> anyhow::Result<serde_json::Value> {
        let res = self.sync_send(&Command::Snapshot { snapshot: SnapshotFile::new()? });
        match res {
            Some(CommandResult::SnapshotResult(Ok(mut snapshot))) => {
                snapshot.read()
            },
    ...
    fn restore(&mut self, data: serde_json::Value) -> anyhow::Result<()> {
        let res = self.sync_send(&Command::Restore { snapshot: SnapshotFile::from_data(data)? });
        match res {
