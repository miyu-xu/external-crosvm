...
/// IrqChip implementation where the entire IrqChip is emulated by XHEE.
///
/// This implementation will use the XHEE API to create and configure the in-kernel irqchip.
pub struct XheeKernelIrqChip {
    pub(super) vm: XheeVm,
    pub(super) vcpus: Arc<Mutex<Vec<Option<XheeVcpu>>>>,
...
impl XheeKernelIrqChip {
    /// Construct a new XheeKernelIrqchip.
    pub fn new(vm: XheeVm, num_vcpus: usize) -> Result<XheeKernelIrqChip> {
        ...
        // prepare xhee_create_device and call ioctl
        let device_dis = xhee_create_device {
            dev_type: xhee_device_type_XHEE_DEV_TYPE_ARM_VGIC_V3_DIST,
            id: 0,
            flags: 0,
        ...
        match vm.create_xhee_device(device_dis) {
            Ok(()) => {}
            Err(e) => {
                error!("failed to create xhee device with err: {}", e);
                return Err(e);
            }
        };

        let device_redis = xhee_create_device {
            dev_type: xhee_device_type_XHEE_DEV_TYPE_ARM_VGIC_V3_REDIST,
            id: 0,
            flags: 0,
        ...
        match vm.create_xhee_device(device_redis) {
            Ok(()) => {}
            Err(e) => {
                error!("failed to create xhee device with err: {}", e);
                return Err(e);
    ...
    /// Attempt to create a shallow clone of this aarch64 XheeKernelIrqchip instance.
...
impl IrqChipAArch64 for XheeKernelIrqChip {
...
/// This IrqChip only works with Xhee so we only implement it for XheeVcpu.
impl IrqChip for XheeKernelIrqChip {
    /// Add a vcpu to the irq chip.
    fn add_vcpu(&mut self, vcpu_id: usize, vcpu: &dyn Vcpu) -> Result<()> {
        let vcpu: &XheeVcpu = vcpu
            .downcast_ref()
            .expect("XheeKernelIrqChip::add_vcpu called with non-XheeVcpu");
    ...
    /// For the XheeKernelIrqChip, the kernel handles listening to irq events being triggered
    ...
    /// For the XheeKernelIrqChip this simply calls the XHEE_SET_IRQ_LINE ioctl.
    ...
    /// This function should never be called on XheeKernelIrqChip.
    fn service_irq_event(&mut self, _event_index: IrqEventIndex) -> Result<()> {
        error!("service_irq_event should never be called for XheeKernelIrqChip");
        Ok(())
    }

    /// Broadcast an end of interrupt.
    /// This should never be called on a XheeKernelIrqChip because a Xhee vcpu should
    /// never exit with the XHEE_EXIT_EOI_BROADCAST reason when an in-kernel irqchip exists.
    fn broadcast_eoi(&self, _vector: u8) -> Result<()> {
        error!("broadcast_eoi should never be called for XheeKernelIrqChip");
        Ok(())
    }

    /// Injects any pending interrupts for `vcpu`.
    /// For XheeKernelIrqChip this is a no-op because Xhee is responsible for injecting
    /// all interrupts.
    ...
    /// For XheeKernelIrqChip this is a no-op because Xhee handles VCPU blocking.
    ...
    /// For XheeKernelIrqChip this is a no-op and always returns Runnable because Xhee
    ...
    fn try_clone(&self) -> Result<Self> {
        // Because the XheeKernelIrqChip struct contains arch-specific fields we leave the
    ...
    /// XheeKernelIrqChip does not need to do anything here.
    fn finalize_devices(
        &mut self,
    ...
    /// The XheeKernelIrqChip doesn't process irq events itself so this function does nothing.
